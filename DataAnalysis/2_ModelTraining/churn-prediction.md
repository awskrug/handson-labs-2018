...
### 데이터 분석
  ...
  - 7줄 - 그럼 각 컬럼은 이탈 여부와는 어떤 관계가 있는지 교차 분석을 합니다.
    ...
    !!hist(by='Churn?', bins=30) bins? 구분을 30개로 해서 히스토그램을 생성해라.
  ...
  - 8줄 - 모든 컬럼별 상관관계를 피어슨 상관계수(Pearson correlation coefficient)로 계산(pd.corr)하여 보여주고,
    ...
    > 참고로 피어슨 상관계수는 1과 가까울수록 양의 상관관계이며(완전 동일), 0은 완전 다름, -1과 가까울수록 음의 상관관계(반대방향으로 완전 동일)입니다. r = x 와 y 가 함께 변하는 정도 / x 와 y 가 각각 변하는 정도
    from http://gomguard.tistory.com/173

    100% 일치하는 것으로 보이는 컬럼은 제거합니다. 딱 봐도 많이 썼으니까 많이 나오는 당연한 결과입니다만, 보통 학습된 데이터에만 맞는 과적합(overfit) 등의 문제를 일으킬 수 있기 때문에 다음 컬럼은 9줄에서 제거합니다.
...
### 예측 모델 평가
  - 19줄 - 예측 모델 평가
    ...
    **비용 할당**
    ...
    1. TN 은 $0 이라 가정한다. 유지할 고객만 정확히 판단하면 더 할 것도 없다.

    2. FP 가 문제다. 이탈 고객을 유지할 것이라 잘못 예측했고, 고객을 잃고, 이익과 광고 비용과 관리 비용, 판매 비용, 장비 비용 등 다른 고객을 유치할 비용을 지출해야 한다. 통밥으로 보면 대략 몇백 달라의 손해를 본다. 그러니 FN에 따른 손해비용을 $500 로 가정하자.

    3. 이탈할 것으로 예측한 고객에게 들어가는 해지방어를 위한 비용을 \$100라 하자.
...
### 예측 모델 확장
  ...
  - 가입 유지 인센티브를 받는 일부 고객은 여전히 ​​변동될 것입니다. 비용 함수(COST(C))에 인센티브를 받았음에도 불구하고 이탈 확률을 포함하면 고객 유치 프로그램에 더 높은 투자효율이 발생할 수 있습니다.
  - 요금제를 저가로 전환하거나 유료기능을 비활성화 한 고객은 별도로 모델링 할 수 있는 다양한 종류의 이탈 방식을 적요합니다.
  - 고객 행동의 진화 모델링. 사용량이 감소하고 고객 서비스에 걸려 오는 전화 수가 증가하는 경우 추세가 반대라면 변동될 수 있습니다. 고객 행동 동향은 수시로 통합해야합니다.
  - 실제 교육 자료 및 금전적 비용 할당은 훨씬 더 복잡할 수 있습니다.
  - 각 유형의 이탈 유형에 대한 다양한 모델이 필요할 수 있습니다.

  추가적인 복잡성에 관계없이 유사한 원리가 적용될 수 있습니다.
